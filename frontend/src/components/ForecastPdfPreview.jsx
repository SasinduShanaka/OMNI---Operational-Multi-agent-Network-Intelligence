    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          {url && <iframe key={view} ref={frame} title="Demand forecast PDF" src={`${url}#page=1&view=${view}&toolbar=0&navpanes=0`} onLoad={() => setReady(true)} className="min-h-0 w-full flex-1 basis-0 border-0 bg-[#eef2f0]" />}
          <footer className="flex shrink-0 justify-between border-t border-slate-200 bg-white px-5 py-2.5 text-[10px] text-slate-400"><span>OMNI demand intelligence</span><span>A4 / {report.pdf.getNumberOfPages()} page</span></footer>
        </div>
      </div>
    </div>
  </dialog>
