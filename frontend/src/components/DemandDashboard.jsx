...
export default function DemandDashboard({ sku, setSku, periods, setPeriods, products, result, isLoading, error, requestForecast, setActivePage }) {
  ...
  return <main className="min-h-full bg-[#f3f6f5] px-4 py-6 text-slate-800 sm:px-6 lg:px-8">
    ...
    <div className="mx-auto max-w-[1440px] space-y-5">
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#102e2b] to-emerald-950 p-5 text-white sm:p-6">
        ...
        <div className="relative grid gap-5 lg:grid-cols-[1.2fr_1fr]">
          <div><div className="flex flex-wrap items-center gap-3"><span className="rounded-md border border-white/20 px-2 py-1 text-[10px] font-semibold tracking-widest text-emerald-200">{result?.sku || sku || 'PRODUCT'}</span><p className="text-sm text-emerald-50/80">{result?.product_name || selectedName || 'Select a product'}</p></div>
            ...
            <div className="mt-2 flex items-baseline gap-3"><strong className="text-4xl font-semibold tracking-tight sm:text-5xl">{result ? number(result.forecast) : '—'}</strong><span className="text-sm text-emerald-100/60">units forecast</span></div>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-emerald-700">02 / Production outlook</p><div className="mt-2 flex items-center justify-between gap-2"><h2 className="text-xl font-bold tracking-tight">Monthly plan</h2><span className="rounded-lg bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700">{result?.predictions.length || 0} MONTHS</span></div>
        ...
        <section className="grid gap-6 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        <div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-emerald-700">Model / Method & validation</p><h2 className="mt-2 text-xl font-bold tracking-tight">Behind the forecast</h2><p className="mt-3 text-xs leading-6 text-slate-500">Holt’s model follows the demand level and trend. Historical backtests measure past performance, not the probability of a correct future prediction.</p></div>
...
function Stat({ label, value, unit, detail, color }) {
  ...
  return <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><h2 className="text-xs font-medium text-slate-500">{label}</h2><span className={`h-2 w-2 rounded-full ${colors[color]}`} /></div><p className="mt-3 text-2xl font-semibold tracking-tight tabular-nums text-slate-900">{value} <span className="text-[11px] font-normal tracking-normal text-slate-400">{unit}</span></p><p className="mt-3 text-[11px] leading-5 text-slate-500">{detail}</p></section>
}
